...
-- One-click FCPXML roundtrip script for resetting VFX NAMING title numbers back to XXXX.
--
-- Rules:
-- - Works on the CURRENT OPEN PROJECT only
-- - Exports current project to temp FCPXML
-- - Finds title clips whose title NAME contains "VFX NAMING"
-- - Reads the first visible text line inside the title
-- - If the first line matches: <BASE>_dddd
--   then dddd is replaced with XXXX
-- - Imports patched XML back into Final Cut
-- - Deletes temp files
--
-- Examples:
--   SMG_SC022_0010 -> SMG_SC022_XXXX
--   SMG_SC022_0140 -> SMG_SC022_XXXX
--   ABC_SC105_0030 -> ABC_SC105_XXXX
--
-- Recommended template inside each VFX title:
--   SMG_SC022_0010
--   CG สมิง
...
local CONFIG = {
    ...
    LOG_LIMIT = 120,
    TEMP_BASENAME = "vfx_reset_naming",
}

local function log(msg) print("[vfx-reset-naming] " .. tostring(msg)) end
...
local function compute_reset_plan(titles)
    local plan = {}

    for _, t in ipairs(titles) do
        local base = t.first_line:match("^(.-)_(%d%d%d%d)$")
        if base then
            plan[#plan+1] = {
                title = t,
                old_code = t.first_line,
                new_code = base .. "_" .. CONFIG.PLACEHOLDER,
            }
        end
    end
...
local function main()
    ...
    local plan = compute_reset_plan(titles)
    log("Titles matching _dddd: " .. tostring(#plan))
    ...
    if CONFIG.DRY_RUN_PARSE then
        ...
        if sk and sk.toast then sk.toast("VFX Reset Naming parse dry run complete", 4) end
        return
    end
    ...
    if sk and sk.toast then sk.toast("VFX Reset Naming finished", 5) end
end

local ok, err = pcall(main)
if not ok then
    log("ERROR: " .. tostring(err))
    if sk and sk.toast then sk.toast("VFX Reset Naming failed: " .. tostring(err), 6) end
end
