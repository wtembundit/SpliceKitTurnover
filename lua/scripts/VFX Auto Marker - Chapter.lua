-- splicekit_vfx_pipeline_generic_chapter.lua
-- Generic one-click VFX marker pipeline for SpliceKit / Final Cut Pro
-- Variant marker kind: chapter
--
...
local CONFIG = {
    ...
    LOG_LIMIT = 120,
    TEMP_BASENAME = "splicekit_vfx_pipeline_generic_chapter",
...
local function log(msg) print("[vfx-generic-chapter] " .. tostring(msg)) end
...
local function main()
    ...
    if sk and sk.toast then sk.toast("Generic VFX Chapter finished", 5) end
end

local ok, err = pcall(main)
if not ok then
    log("ERROR: " .. tostring(err))
    if sk and sk.toast then sk.toast("Generic VFX Chapter failed: " .. tostring(err), 6) end
end
