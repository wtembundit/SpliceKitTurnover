-- splicekit_vfx_pipeline_generic_standard.lua
-- Generic one-click VFX marker pipeline for SpliceKit / Final Cut Pro
-- Variant marker kind: standard
--
-- This version fixes marker TYPE at LIVE CREATION time by calling the correct
-- timeline action:
--   standard -> addMarker
--   todo     -> addTodoMarker
...
local CONFIG = {
    ...
    LOG_LIMIT = 120,
    TEMP_BASENAME = "splicekit_vfx_pipeline_generic_standard",

    MARKER_KIND = "standard",
}
...
local function log(msg) print("[vfx-generic-standard] " .. tostring(msg)) end
        ...
        if i > 1 then
            ...
            if gap > CONFIG.MAX_REASONABLE_GAP then
                return false, string.format("Suspiciously large gap between %s and %s: %s",
                    events[i-1].marker_name, e.marker_name, fmt_seconds(gap))
            end
        end
    end
    return true, "ok"
...
local function parse_existing_markers(xml)
    ...
    table.sort(markers, function(a,b)
        if a.interval_start == b.interval_start then
            return a.parent_duration < b.parent_duration
        end
...
local function build_marker_xml_for_kind(m, new_value, new_note, marker_kind)
    local attrs = {}
    for k, v in pairs(m.attrs) do attrs[k] = v end

    attrs.value = new_value
    if new_note and new_note ~= "" then attrs.note = new_note else attrs.note = nil end

    local element_name = "marker"

    if marker_kind == "standard" then
        attrs.completed = nil
        attrs.posterOffset = nil
        element_name = "marker"
    elseif marker_kind == "todo" then
        attrs.completed = "0"
        attrs.posterOffset = nil
        element_name = "marker"
    elseif marker_kind == "chapter" then
        attrs.completed = nil
        if attrs.posterOffset == nil then attrs.posterOffset = attrs.start end
        element_name = "chapter-marker"
    else
        error("Unknown marker kind: " .. tostring(marker_kind))
    end

    local ordered, seen = {}, {}
    local preferred = {"start","duration","value","completed","note","posterOffset"}
    ...
    return "<" .. element_name .. " " .. table.concat(parts, " ") .. "/>"
end
...
local function marker_action_for_kind(kind)
    if kind == "standard" then return "addMarker" end
    if kind == "todo" then return "addTodoMarker" end
    if kind == "chapter" then return "addChapterMarker" end
    error("Unknown MARKER_KIND: " .. tostring(kind))
end

local function run_live_create(events, kind)
    log("=== LIVE CREATE PHASE ===")
    log("Configured marker kind: " .. tostring(kind))
    local action = marker_action_for_kind(kind)
    local created = 0
    ...
    for i, e in ipairs(events) do
        ...
        if not CONFIG.DRY_RUN_LIVE then
            ...
            if res and res.error then
                error("timeline.action failed: " .. tostring(res.error) .. " action=" .. tostring(action))
            end
            sk.sleep(CONFIG.LIVE_SLEEP_SEC)
            created = created + 1
        end
...
local function run_xml_relabel(events, kind)
    ...
    if #markers < floor_count then
        error("Export still contains too few markers (" .. tostring(#markers) .. ", need >= " .. tostring(floor_count) .. ").")
    end
    ...
    for _, e in ipairs(events) do
        ...
        if choice then
            ...
            replacements[#replacements+1] = {
                ...
                replacement = build_marker_xml_for_kind(choice.marker, e.marker_name, e.marker_note, kind),
                event = e, marker = choice.marker, score = choice.score,
            }
            if matched <= CONFIG.LOG_LIMIT then
                log(string.format(
                    'match event="%s" t=%s -> marker value="%s" interval=%s..%s parent=%s score=%.3f kind=%s',
                    ...
                    choice.marker.parent_tag, choice.score, kind
                ))
            end
        else
...
local function marker_kind()
    local k = lower(trim(CONFIG.MARKER_KIND))
    if k ~= "standard" and k ~= "todo" and k ~= "chapter" then
        error('MARKER_KIND must be "standard", "todo", or "chapter"')
    end
    return k
end
...
local function main()
    ...
    if sk and sk.toast then sk.toast("Generic VFX Standard finished", 5) end
end

local ok, err = pcall(main)
if not ok then
    log("ERROR: " .. tostring(err))
    if sk and sk.toast then sk.toast("Generic VFX Standard failed: " .. tostring(err), 6) end
end
